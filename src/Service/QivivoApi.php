...
# Thanks to https://github.com/KiboOst/php-simpleQivivoAPI
class QivivoApi
{
    ...
    public function getCurrentProgram() //@return['result'] array with current program selection, multizone supported
    {
        ...
        foreach ($programs as $prog) {
            if ($ID == $prog['id']) {
                $result['Zone Thermostat'] = $prog['name'];
            }
        }
    ...
    public function getSynthesis($dateStart, $dateEnd)
    {
        ...
        if (!isset($jsonAnswer['success'])) {
            return array('result' => null, 'error' => $jsonAnswer);
        }
    ...
    public function setTemperature($value, $programName = false)
    {
        ...
        if ($programName) {
            ...
            if (!$myID) {
                return array('result' => null, 'error' => 'Sorry, could not find program ' . $programName);
            }
        }
        ...
        $answer = $this->_request('POST', $url, $post, $xmlRequest = true);
        $jsonAnswer = json_decode($answer, true);
        if (!isset($jsonAnswer['success']['state'])) {
            return array('result' => null, 'error' => $jsonAnswer);
        }
    ...
    public function setTempSettings(
        $code = '',
        $value = ''
    ) //@code in availValues | @return['result'] true, @return['error'] if any
    {
        $availValues = ['pres_1', 'pres_2', 'pres_3', 'pres_4', 'confort', 'nuit', 'hg', 'absence'];
        if (!in_array($code, $availValues)) {
            return array('result' => null, 'error' => 'Got wrong value, should be in ' . implode(', ', $availValues));
        }
    ...
    public function setDepartureAlert($days = 3
    ) //@days in availValues | @return['result'] true, @return['error'] if any
    {
        //$url = $this->_urlRoot.'/update-alert'; ??
        $availValues = [1, 1.5, 2, 2.5, 3, 3.5, 4];
        if (!in_array($days, $availValues)) {
            return array('result' => null, 'error' => 'Got wrong value, should be in ' . implode(', ', $availValues));
        }
    ...
    ) //@name string, @datas array | @return['result'] true, @return['error'] if any
    {
        ...
        if (!$myID) {
            return array('result' => null, 'error' => 'Sorry, could not find program ' . $name);
        }
        ...
        foreach ($datas as $day) {
            ...
            if ($jsonAnswer['success']['state'] != 'success') {
                return array('result' => null, 'error' => $jsonAnswer);
            }
            $dayId += 1;
        }
    ...
    protected function _request($method, $url, $post = null, $xmlRequest = false)
    {
        if (!isset($this->_curlHdl)) {
            ...
            curl_setopt($this->_curlHdl, CURLOPT_USERAGENT,
